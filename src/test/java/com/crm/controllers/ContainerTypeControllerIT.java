...
class ContainerTypeControllerIT {
  ...
  void createContainerType_thenStatusOk_andReturnCombined() throws JsonProcessingException, Exception {
    // given
  ...
  @Test
  @WithMockUser(username = "moderator", roles = { "MODERATOR" })
  void getContainerType_thenStatusOk_andReturnCombined() throws JsonProcessingException, Exception {
    // given
  ...
  void searchContainerTypes_thenStatusOk_andReturnInbounds() throws Exception {
    // given
  ...
  @Test
  @WithMockUser(username = "forwarder", roles = { "FORWARDER" })
  void getContainerTypes_thenStatusOk_andReturnCombineds() throws JsonProcessingException, Exception {
    // given
  ...
  @Test
  @WithMockUser(username = "moderator", roles = { "MODERATOR" })
  void editContainerType_thenStatusOk_andReturnBid() throws Exception {
    // given
    ...
    MvcResult result = mockMvc
        ...
        .andDo(print()).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.description").value("123456")).andReturn();
  ...
  void deleteContainerType_thenStatusOk_AndReturnMessage() throws Exception {
    ...
    MvcResult result = mockMvc.perform(delete("/api/container-type/1").contentType(MediaType.APPLICATION_JSON_VALUE))
        ...
        .andExpect(jsonPath("$.message").value("Container Type has remove successfully")).andReturn();
